catOmega <- function(dat, estimator = "DWLS") {
  library(lavaan)
  library(mnormt)
  library(tidyverse)
  ###############################################################################
  ...
  fit <- cfa(model, data = dat, se = "none", ordered = varnames, estimator = estimator)
  ...
  # error <- try(fit <- lavaan::cfa(model, data = dat, se = "none", 
  #                                 ordered = varnames, estimator = estimator), silent = TRUE)
  # converged <- FALSE
